...
class bank {
  ...
  transaction(obj: bankAccount) {
    let newAccount = this.account.filter(
      (acc) => acc.accNumber !== obj.accNumber
    );
  ...
  do {
    ...
    if (service.select === "View Balance") {
      ...
      if (account) {
        let name = meezanBank.customer.find(
          (item) => item.accountNumber == account?.accNumber
        );
        console.log(
          chalk.italic.bold.cyan(
            "<------------------------------------------------------->"
        ...
        console.log(
          `Dear ${chalk.green.italic(name?.firstName)} ${chalk.green.italic(
            name?.lastName
          )} your Account balance is ${chalk.bold.blue.italic(
            `$${account.balance}`
          )} `
        );
        console.log(
        ...
        if (ans.rupee <= account.balance) {
          ...
          console.log(
            chalk.bold.italic.magenta(
              `<---- Withdraw ${chalk.green(
                ans.rupee
              )} from account ${chalk.gray(
                account.accNumber
              )}. New balance: ${chalk.bold.green(newBalance)} ---->`
            )
          );
          console.log(
            chalk.italic.bold.cyan(
    ...
    if (service.select === "Cash Deposit") {
      ...
      if (account) {
        ...
        console.log(
          chalk.bold.italic.magenta(
            `---- Debited ${chalk.green(ans.rupee)} from account ${chalk.gray(
              account.accNumber
            )}. New balance: ${chalk.bold.green(newBalance)} ----`
          )
        );
